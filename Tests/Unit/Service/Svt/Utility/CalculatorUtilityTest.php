...
class CalculatorUtilityTest extends TestCase
{


    /**
     * @var CalculatorUtility
     */
    protected $subject = NULL;

    public function setUp()
    {
        $this->subject = new CalculatorUtility();
    }
    ...
    public function calculatePrepareUsedADefindeListOfTokenFunctionArray()
    {
        ...
        $this->assertSame(
            11,
            count($result),
            '1. The constant is changed. It`s a hit, that the class may not workl like expected. (10 plus 1 [Minus-operator])) '
        );
...
//            );
    ...
    public function testEndOfCalculationArrivedReturnBooleanGivenOfInternalSettings($message, $expect, $params)
    {
        if (!isset($expect) && empty($expect)) {
            ...
            $reflection = new \ReflectionClass($calculatorUtility);
            $reflection_calcByteLength = $reflection->getProperty('calcByteLength');
            $reflection_calcByteLength->setAccessible(true);
            $reflection_calcByteLength->setValue($calculatorUtility, $params['length']);
            $reflection_pointNext = $reflection->getProperty('pointNext');
            $reflection_pointNext->setAccessible(true);
            $reflection_pointNext->setValue($calculatorUtility, $params['pointer']);

            $result = $calculatorUtility->testEndOfCalculationArrived();
    ...
    public function testForEndBracketReturnBooleanGivenOfInternalSettings()
    {
        ...
        $this->assertSame(
            ')',
            $calculatorUtility::STRING_ENDBRACKET_NORMAL,
        ...
        $reflection_calcByteLength->setAccessible(true);
        $reflection_calcByteLength->setValue($calculatorUtility, 'h');

        $result = $calculatorUtility->testForEndBracket();
        $this->assertSame(
            false,
            $result,
            '2. The current-value `h` is not identical to `)`.'
        );
        $reflection_calcByteLength->setValue($calculatorUtility, ')');
    ...
    public function testForFlagOkayReturnBooleanGivenOfInternalSettings()
    {
        ...
        $reflection_flagOkay->setValue($calculatorUtility, false);

        $result = $calculatorUtility->testForFlagOkay();
        $this->assertSame(
            false,
            $result,
            '1. The current reflected value in flagOkay `false` is identical to the result `false`.'
        );
        $reflection_flagOkay->setValue($calculatorUtility, true);
    ...
    public function resetForCalculationConfigureTheCorrectInternalConfiguration($message, $expects, $params)
    {
        if (!isset($expects) && empty($expects)) {
            ...
            /** @var CalculatorUtility $calculatorUtility */
            $calculatorUtility = new CalculatorUtility();

            if ($params['start'] !== null) {
                $calculatorUtility->resetForCalculation($params['start']);
            }
            $calculatorUtility->resetForCalculation($params['test']);

            $reflection = new \ReflectionClass($calculatorUtility);
            $reflection_pointNext = $reflection->getProperty('pointNext');
            $reflection_pointNext->setAccessible(true);
            $pointNext = $reflection_pointNext->getValue($calculatorUtility);

            $reflection_pointByte = $reflection->getProperty('pointByte');
            $reflection_pointByte->setAccessible(true);
            $pointByte = $reflection_pointByte->getValue($calculatorUtility);

            $reflection_calcByteArray = $reflection->getProperty('calcByteArray');
            $reflection_calcByteArray->setAccessible(true);
            $calcByteArray = $reflection_calcByteArray->getValue($calculatorUtility);

            $reflection_calcByteLength = $reflection->getProperty('calcByteLength');
            $reflection_calcByteLength->setAccessible(true);
            $calcByteLength = $reflection_calcByteLength->getValue($calculatorUtility);

            $reflection_flagOkay = $reflection->getProperty('flagOkay');
            $reflection_flagOkay->setAccessible(true);
            $flagOkay = $reflection_flagOkay->getValue($calculatorUtility);
    ...
    public function resetForCalculationConfigureANArrayWithOngoingIndexing()
    {
        ...
        $calculatorUtility->resetForCalculation($testString);
    ...
    public function nextTokenConfigureTheCorrectInternalConfiguration($message, $expects, $params)
    {
        if (!isset($expects) && empty($expects)) {
            ...
            $reflection_pointNext = $reflection->getProperty('pointNext');
            $reflection_pointNext->setAccessible(true);
            $reflection_pointNext->setValue($calculatorUtility,$params['pointNext']);

            $reflection_pointByte = $reflection->getProperty('pointByte');
            $reflection_pointByte->setAccessible(true);
            $reflection_pointByte->setValue($calculatorUtility, $params['pointByte']);

            $reflection_flagOkay = $reflection->getProperty('flagOkay');
            $reflection_flagOkay->setAccessible(true);
            $reflection_flagOkay->setValue($calculatorUtility,$params['flagOkay']);

            $calculatorUtility->nextToken();
            $pointNext = $reflection_pointNext->getValue($calculatorUtility);
            $pointByte = $reflection_pointByte->getValue($calculatorUtility);
            $flagOkay = $reflection_flagOkay->getValue($calculatorUtility);
        ...
        return [
            ...
            [
                ...
                [
                    'flagOkay' => false,
                    'result' => 0,
                    // google will calculate 22797606976.3 for this. the windows calculator
    ...
    public function detectOperatorReturnEmptyStringAndSetInternalConfigurationToFalseGivenOperatorDoesNotExistOnCurrentPlace()
    {
        ...
        $calculatorUtility = $this->getMockBuilder(CalculatorUtility::class)
            ->setMethods(['nextToken'])
            ->getMock();

        $calculatorUtility->expects($this->never())
            ->method('nextToken');
        // reset to default-Values
        $calculatorUtility->resetForCalculation($testString);

        $operator = $calculatorUtility->detectOperator();
    ...
    public function detectOperatorReturnOperatoAndSetInternalConfigurationGivenOperatorExist()
    {
        ...
        $calculatorUtility->expects($this->once())
            ->method('nextToken');
        // reset to default-Values
        $calculatorUtility->resetForCalculation($testString);
    ...
    public function testForStartBracketReturnFalseAndLeavedInternalConfigurationGivenTheCurrentPositionDoesNotContainAnOpeningBracket()
    {
        ...
        $flagTest = $calculatorUtility->testForStartBracket();
    ...
    public function detectEndBracketReturnNothingAndLeavedInternalConfigurationUnchangedGivenTheCurrentPositionDoesNotContainAnOpeningBracket()
    {
        ...
        $calculatorUtility->detectEndBracket();
    ...
    public function dataProviderCalculatePairForGivenCalculationScenarios()
    {
        return [
            ...
            [
                ...
                'if the divisor ist near zero (between ] -'.CalculatorUtility::PRECISION_DELTA_EXPSILON .' , ' .
                CalculatorUtility::PRECISION_DELTA_EXPSILON.' [',
                [
                    [true, '5', '/', '4', '1.25'],
                    [true, '5', '/', '-4', '-1.25'],
                    [true, '-5', '/', '4', '-1.25'],
                    ...
                    [true, '3', '/', '0.001', '3000'],
                    [true, '3', '/', '0.0001', '30000'],
                    [false, '3', '/', CalculatorUtility::PRECISION_DELTA_EXPSILON/2, '0'],
                    [false, '-3', '/', CalculatorUtility::PRECISION_DELTA_EXPSILON/2, '0'],
                    [false, '3', '/', -CalculatorUtility::PRECISION_DELTA_EXPSILON/2, '0'],
                    [false, '-3', '/', -CalculatorUtility::PRECISION_DELTA_EXPSILON/2, '0'],
                ]
            ...
            [
                ...
                'if the divisor is near zero (between ] -' . CalculatorUtility::PRECISION_DELTA_EXPSILON . ' , ' .
                CalculatorUtility::PRECISION_DELTA_EXPSILON . ' [' . "\n" .
    ...
    public function defineFirstNumberInTermCallMockedMethodDefineSecondNumberAfterOperatorInTermIfNoParameterIsDefined()
    {
        ...
        $operator = $calculatorUtility->defineFirstNumberInTerm();
    ...
    * @return float|int
    */
    public function calculatePairAndInitPerhapsLazyRecursion($firstNumber, $operator, $secondNumber)
    {
        $priority = $this->definePriorityOperator($operator);
        if (
            ...
            if ($testPriority === $priority) {
                ...
                $secondNumber = $this->calculateOperator($secondNumber);
                $firstNumber = $this->calculatePair($firstNumber, $operator, $secondNumber);
            } elseif ($testPriority < $priority) {
                $firstNumber = $this->calculatePair($firstNumber, $operator, $secondNumber);
                $priority = $testPriority; // downgrade the operator-Priority of the current process
            } else {
                $this->flagOkay = $this->flagOkay && false; // this place should never arrived
            }
        }
        return $firstNumber;
    }
    ...
    public function calculatePairAndInitPerhapsLazyRecursionCallMockedMethodToCheckDifferentSituations($message, $expects, $params)
    {
        ...
        $calculatorUtility->expects($this->exactly($params['priorCount']))
            ->method('definePriorityOperator')
            ->with($this->anything())
            ->will($this->onConsecutiveCalls($params['priorFirst'],$params['priorSecond']));

        $calculatorUtility->expects($this->exactly($params['arriveEndCount']))
            ->method('testEndOfCalculationArrived')
            ->willReturn($params['arriveEnd']);
        $calculatorUtility->expects($this->exactly($params['arriveBracketCount']))
            ->method('testForEndBracket')
            ->willReturn($params['arriveBracket']);

        $calculatorUtility->expects($this->exactly($params['calculateOperatorCount']))
            ->method('calculateOperator')
            ->willReturn($params['calculateOperatorValue']);



        $result = $calculatorUtility->calculatePairAndInitPerhapsLazyRecursion(
            $params['first'],
            $params['operator'],
            $params['second']
        );
    ...
    public function calculateReturnResultIfFlagOkayTrueIfNoErrorDetectedAndEverythingAnalysedOtherwiseRetunrnFalse($message, $expects, $params)
    {
        ...
        $reflection_calcByteLength = $reflection->getProperty('calcByteLength');
        $reflection_calcByteLength->setAccessible(true);
        $reflection_calcByteLength->setValue($calculatorUtility, $params['calcByteLength']);

        $result = $calculatorUtility->calculate('DummyStringForMockedMethod');
        $this->assertSame(
    ...
    public function calculateOperator($firstValue = null)
    {
        $result = 0;
    ...
    public function calculateOperatorReturnResultIfFlagOkayTrueIfNoErrorDetectedAndEverythingAnalysedOtherwiseRetunrnFalse($message, $expects, $params)
    {
        ...
        $result = $this->subject->calculateOperator($params['calculateOperatorParam']);
        $this->assertSame(
